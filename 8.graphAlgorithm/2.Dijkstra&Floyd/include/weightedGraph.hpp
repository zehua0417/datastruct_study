...
public:
    ...
    int num_vertices;
    int num_edges;
    std::vector<std::list<WeightedEdge<T> > > matrix;

    WeightedGraph(WeightedGraph<T> const &graph) {
        num_vertices = graph.num_vertices;
        num_edges = graph.num_edges;
        //matrix = graph.matrix;
        matrix = std::vector<std::list<WeightedEdge<T>>>(graph.matrix);
    }
    ...
    void insertEdge(std::pair<int, WeightedEdge<T>&> edge) {
        insertEdge(edge.first, edge.second);
    }

    int searchLinkedVertex(int& vertex1, int& vertex2) const {
        for (int i = 0; i < matrix.size(); i++){
            for(auto edge : matrix[vertex1]){
                if(edge.linked_vertex == vertex2)
                    return edge.weight;
            }
        }
        return -1;
...
private:
    ...
    typename std::list<WeightedEdge<T>>::const_iterator currentEdgeIterator;  // 当前顶点邻接表中的边迭代器
